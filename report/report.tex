...
\subsubsection{Complexity}
\begin{itemize}
	\item[] \textbf{Time complexity}: $O(b^{f^*(s)-f(s)})$, where $b$ is the branching factor and $f^*(s)$ is the actual cost from root $s$ to goal node $g$.
...
To assess the empirical space complexity, we measure the number of generated states and the maximum heap size \footnote{\label{heapnote}In the context of BFS, maximum heap size means maximum queue length.}. Note that since we adopt graph search, the memory consumption consists of both the \texttt{explored} set (reflected by number of generated states) and the \texttt{frontier} (reflected by maximum heap size).

To assess the empirical time complexity, we measure the search depth and runtime. Additionally, we measure the number of explored states and solution depth to further evaluate the efficiency \footnote{It should explore as few nodes in the frontier as possible.} of the informed search heuristics.
...
\begin{table}[h]
	...
	\begin{tabular}{l|c|c|c|c|c|c|c|c|c|c|c|c} \hline
		...
		Avg Solution Depth & 14.6 & 14.6 & 14.6 & 14.6 & - & 25.4 & 28.2 & 28.2 & - & 28.5 & 33.8 & 33.8 \\\hline
		Avg Search Depth & 14.6 & 14.6 & 14.6 & 14.6 & - & 25.4 & 28.2 & 28.2 & - & 28.5 & 33.8 & 33.8 \\\hline
		Avg Explored States & 10461.9 & 594.5 & 119.6 & 68.5 & - & 93122.4 & 25607.9 & 3486.7 & - & 95769 & 63719 & 9652.6 \\\hline
		Avg Generated States & 10461.9 & 948.6 & 197.2 & 116 & - & 177991.7 & 48460.2 & 6692.5 & - & 208102.5 & 136932.4 & 21830 \\\hline
		Avg Heap size\footnotemark[\ref{heapnote}] & 3516.4 & 356.2 & 79.2 & 48.8 & - & 85254.9 & 23307.5 & 3270.5 & - & 112753 & 74193.9 & 12291.1 \\\hline
		Avg Runtime(s) & 0.201 & 0.0931 & 0.0773 & 0.0881 & - & 4.78 & 2.14 & 1.03 & - & 5.68 & 7.52 & 5.28 \\\hline
...
\end{table}
\vspace*{-30pt}
From the results, we can see that while all 4 algorithms manage to solve all 10 $3 \times 3$ puzzles, BFS had TLE for all $4 \times 4$ and $5 \times 5$ test cases.

Among the 3 informed search heuristics, Manhattan Distance (MD) and Linear Conflict (LC) performed better than Misplaced Tile (MT) in terms of pass rate and both time and space complexity since MT is optimal to the most relaxed problem \footnote{Tiles can move to any place in one move.}. Between MD and LC, LC outperformed MD in terms of both time and space complexity. This is consistent with the fact that LC dominates MD. There is an exception for the runtime of $3 \times 3$, this is most likely due to the combination of small absolute difference (~60 rather than 10000+ for other cases) in generated/explored states and overhead incurred in computing the estimated cost using LC, as it has a complexity of $O(n^3)$, where $n$ is the dimension of the grid.
